...
public enum TeleportItem
{
    ...
    {
        return hasRequirements() && methods.inventory.getItem(itemIds) != null;
    }
    ...
    public boolean hasRequirements()
    {
        ...
        switch (this)
        {
            case JATIZSO_LYRE:
            case NEITIZNOT_LYRE:
                return hasQuestReqs && methods.client.getVarbitValue(Varbits.DIARY_FREMENNIK_ELITE) > 0;
            case WATERBIRTH_ISLAND_LYRE:
                return hasQuestReqs && methods.client.getVarbitValue(Varbits.DIARY_FREMENNIK_HARD) > 0;
            case TROLL_STRONGHOLD:
                return hasQuestReqs && methods.skills.getRealLevel(Skill.AGILITY) >= 73 && methods.client.getVarbitValue(Varbits.DIARY_FREMENNIK_HARD) > 0;
            case ARDOUGNE_TELEPORT_TAB:
                return hasQuestReqs && methods.client.getVarpValue(165) >= 30;
            case SALVE_GRAVEYARD_TELEPORT_TAB:
                return hasQuestReqs && methods.client.getVarpValue(302) >= 61;
            default:
                return hasQuestReqs;
        }
    }
